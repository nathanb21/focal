...
export function CitationPanel({ citation, open, onOpenChange }: CitationPanelProps) {
  ...
  return (
    <AnimatedSheet open={open} onOpenChange={onOpenChange}>
      <SheetContent forceMount className="w-full max-w-[500px] gap-0 p-0 sm:max-w-[500px]">
        <div className="border-b border-slate-100 px-6 pb-5 pt-7">
          <SheetHeader className="pr-7">
            <div className="mb-4 flex items-center gap-2">
              ...
              <span className="text-[11px] font-semibold uppercase tracking-[0.14em] text-blue-700">Source document</span>
            </div>
            <SheetTitle className="text-[20px] leading-7 tracking-[-0.02em]">{citation.title}</SheetTitle>
            <SheetDescription className="mt-2 flex items-center gap-2 text-[13px]">
        ...
        <div className="flex-1 overflow-y-auto px-6 py-6">
          <div className="rounded-xl border border-blue-100 bg-blue-50/55 p-4">
            ...
            <blockquote className="text-[15px] leading-7 text-slate-700">{citation.excerpt}</blockquote>
          </div>

          <div className="mt-7">
            <p className="mb-3 text-[11px] font-semibold uppercase tracking-[0.14em] text-slate-400">Document details</p>
            <dl className="divide-y divide-slate-100 rounded-xl border border-slate-100 bg-white">
              ...
              <div className="flex items-center justify-between gap-4 px-4 py-3">
                ...
                <dd>
                  <span className={`rounded-full px-2.5 py-1 text-[11px] font-semibold capitalize ring-1 ring-inset ${confidenceStyles[citation.confidence ?? "medium"]}`}>
          ...
          <div className="mt-7 rounded-lg bg-slate-50 px-4 py-3.5 text-[12px] leading-6 text-slate-500">
